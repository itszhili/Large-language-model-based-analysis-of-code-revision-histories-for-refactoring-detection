...
# Function to detect refactorings in a repo using torchrun
def detect_refactorings(repo_name):
    command = f'torchrun main.py getrefs -r "Repos/{repo_name}"'
    subprocess.run(command, shell=True)

# Read the CSV and process each repo
    ...
    for row in reader:
        ...
        if repo_exists(repo_path):
            if not json_exists(repo_name):
                ...
                # Check if JSON file exists after detection
