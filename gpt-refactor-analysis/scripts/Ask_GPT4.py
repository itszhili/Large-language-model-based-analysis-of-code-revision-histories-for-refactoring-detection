...
def classify_with_gpt(content, custom_question=None, model="gpt-4", temperature=0.6, max_tokens=10):
    ...
    try:
        ...
        if custom_question:
            ...
            question = f"{content} According to the typical structure of a machine learning project, which one of these componets[Data Pipeline, Model Development] does this refactoring belongs to? Please provide only the componet name."
...
def main(model: str = "gpt-4", temperature: float = 0.6, max_tokens: int = 10, included_fields=None):
    """
